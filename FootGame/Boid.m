...
#define leftEdge (_border.size.width > 0 ? 0.0f : _border.origin.x)
#define bottomEdge (_border.size.width > 0 ? 0.0f : _border.origin.y)
#define topEdge (_border.size.width > 0 ? 768.0f : _border.size.height)
#define rightEdge (_border.size.width > 0 ? 1024.0f : _border.size.width)
...
@synthesize edgeBehaviorSides=_edgeBehaviorSides;
@synthesize edgeBehaviorTopBottom=_edgeBehaviorTopBottom;
@synthesize doRotation=_doRotation;
#pragma mark Initialization

+(id) spriteWithSpriteFrame:(CCSpriteFrame *)spriteFrame borders: (CGRect) border {
    return [[[self alloc] initWithSpriteFrame:spriteFrame borders:border] autorelease];
}
- (id) init
{
	self = [super init];
	if (self != nil)
	{
		self.maxForce = 0.0f;
		self.maxSpeed = 0.0f;
		self.edgeBehaviorSides = EDGE_NONE;
        self.edgeBehaviorTopBottom = EDGE_NONE;
...
-(id) initWithSpriteFrame:(CCSpriteFrame *)spriteFrame borders:(CGRect)border {
...
-(void) handleBorder
{
	if(_edgeBehaviorSides == EDGE_WRAP)
	{
	...
	else if (_edgeBehaviorSides == EDGE_BOUNCE)
	{
    ...
    if (_edgeBehaviorTopBottom == EDGE_WRAP) {
    ...
    } else if (_edgeBehaviorTopBottom == EDGE_BOUNCE) {
